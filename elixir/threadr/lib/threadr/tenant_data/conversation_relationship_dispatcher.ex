defmodule Threadr.TenantData.ConversationRelationshipDispatcher do
  @moduledoc """
  Supervised worker that periodically recomputes conversation-derived actor relationships.
  """
  ...
  alias Threadr.TenantData.ConversationRelationshipRecompute
  ...
  defp dispatch_pending(batch_size) when is_integer(batch_size) and batch_size > 0 do
    case ControlPlane.list_tenants(context: %{system: true}) do
      ...
      {:error, reason} ->
        Logger.warning("conversation relationship dispatch failed: #{inspect(reason)}")
        :error
    end
  end
  ...
  defp dispatch_tenant(tenant, remaining) do
    ...
    |> Enum.reduce(0, fn conversation_id, count ->
      case ConversationRelationshipRecompute.recompute_conversation_relationships(
             conversation_id,
             tenant.schema_name
           ) do
        {:ok, _result} ->
        ...
        {:error, reason} ->
          Logger.warning(
            "conversation relationship recompute failed for #{tenant.subject_name}:#{conversation_id}: #{inspect(reason)}"
          )
  ...
  defp pending_conversation_ids(tenant, remaining) do
    ConversationRelationshipRecompute.pending_conversation_ids(tenant.schema_name, remaining)
  rescue
    error in Postgrex.Error ->
      Logger.warning(
        "conversation relationship recompute skipped for #{tenant.subject_name}: schema not ready (#{Exception.message(error)})"
      )
